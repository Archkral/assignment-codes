...
class NNPart2:
    def Jacobian(self, inputs, y):
        ...
        for j in range(Nin):  # Order is correct.
            ...
            dvjdIW = - 2 * self.IW[:, j] * (xc ** 2)
            dvjdc = - (self.IW[:, j] ** 2) * -2 * xc
    ...
    def CostFuncPrime(self, inputs, y):
        ...
        Hess = (self.J.T @ self.J)
        dW = -np.linalg.inv((Hess + self.Opts.mu * np.identity(np.size(self.J, axis=1)))) @ self.J.T @ (y - self.yHat)
...
class NNPart2_alg(NNPart1_alg, NNPart2):
    pass


class NNPart2_rand(NNPart1_rand, NNPart2):
    pass
