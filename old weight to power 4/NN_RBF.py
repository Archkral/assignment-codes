...
class NN():
    def forward_rbf(self, X):
        ...
        # Nhidden = np.size(self.centers, axis=0)  # H hidden neurons
        # self.vj = np.zeros([L_end, Nhidden])
        # for i in range(Nin):
        #     wx = np.array([self.IW[:, i]]) * np.array([X[:, i]]).T  # UxH
        #     wc = np.ones([1, L_end]).T * np.array([self.IW[:, i] * self.centers[:, i]])  # UxH
        #     self.vj = self.vj - (wx - wc) ** 2  # i.e. self.vj = negative
        #
        # That loop is equivalent to the following faster implementation. The above is left in for legibility.
        ...
        self.vk = self.yj @ self.LW.T  # UxH @ (OxH).T -> UxH # HxO -> UxOutputs
        yHat = self.vk  # linear function in output neuron so its... yeah
        return yHat
    ...
    def act_func(self, v):
        ...
        return np.exp(-v**2)  # has to be v or -v^2 else its not an RBF but an EBF (exponential basis func)

    def act_func_dz(self, v):
        # derivative of activation function :: sig = np.exp(-z) / ((1 + .np.exp(-z)) ** 2)
        return -2*v*np.exp(-v**2)  # KW:Change -2*v*np.exp(-v**2)  # has to be v or -v^2 else its not an RBF but an EBF
...
########################################################################################################################
# initializations (order is mess cause of inheritance structure, sorry).
...
class NNPart1_alg(NNPart1_rand):
    def __init__(self, Opts, inputs):
        # k-means clustering for centers including PCA pre-processing:
        ...
        # Can even go further and "whiten" the data. But lets keep it simple.
    ...
    def IW_karlo(self, inputs):
        ...
        Goal: Set IW so that at its cluster's mean distance, an amplitude of ~0.9 of its LW is achieved.
        """
        ### Part1: separate duplicates from original centers:
        all_centers = self.centers
        IW_centers = np.around(self.centers, decimals=self.Opts.round)  # rounding
        mod = np.zeros([0, 2])  # stores the order of centers and the modifier for the copies
        IW_used = []
        a = 0
        for n, i in enumerate(IW_centers):
            if tuple(i) not in IW_used:
                IW_used.append(tuple(i))
                mod = np.append(mod, [[1, a]], axis=0)
                a += 1
            else:
                for nn, ii in enumerate(np.array(IW_used)):
                    if tuple(i) == IW_used[nn]:
                        multiplier = 2**mod[mod[:, 1] == nn].shape[0]
                        mod = np.append(mod, [[multiplier, nn]], axis=0)
                        break
        self.centers = np.array(IW_used)

        ### Part2: Do width and anisotropy calculations on the original centers
